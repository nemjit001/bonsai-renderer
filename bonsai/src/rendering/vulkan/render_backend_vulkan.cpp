...
RenderBackend::RenderBackend(Surface const* surface)
    :
    m_impl(new RenderBackendImpl{})
{
...
void RenderBackend::on_resize(uint32_t width, uint32_t height)
{
    vkDeviceWaitIdle(m_impl->device);
...
void RenderBackend::present(FrameState const* frame_state)
{
    ...
    present_info.pImageIndices = &frame_state->swap_image_idx;
